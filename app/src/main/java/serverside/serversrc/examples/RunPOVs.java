package serverside.serversrc.examples;
...
public class RunPOVs {
	...
	public static void main(String[] args) throws JSONException {
		...
		if(user.getRole().equalsIgnoreCase("client")){
			...
			RunFunctions.RunnerPOV(user.getUserID()); //gets runs associated with runner
			...
			if(c.equalsIgnoreCase("post")){
				...
				if(r!=null){
					...
					if(ss.equalsIgnoreCase("Yes")){
						...
						System.out.println("Email: " + runner.getEmail() +
								...
								"\nCompleted Runs: " + runner.getComRunsRunner() + " Average Rating: " + runner.getRunnerRating() + "\n");
						for (Rating rating : runner.getRatings()){
							System.out.println("Reviewer: " + rating.getPostedName() + "| Rating: " + rating.getRating() +
				...
				if(r!=null && r.getCReviewed()==0){
					...
					do{
						...
						rating = new Rating(r.getRunId(),r.getRnerId(), user.getUserName(),ra, review);
				...
				if(r!=null){
					...
					if(ss.equalsIgnoreCase("Yes")){
						...
						System.out.println("Email: " + client.getEmail() +
								...
								"\nCompleted Runs: " + client.getComRunsClient() + " Average Rating: " + client.getRunnerRating() + "\n");
	...
	public static void printRuns(ArrayList<Run> queued, ArrayList<Run> pending, ArrayList<Run> accepted, ArrayList<Run> completed){
		...
		for(Run run : accepted){
			System.out.println("Run Id = " + run.getRunId() + 
					...
					"\nAmount = " + run.getAmount() +
					"\nTime Posted = " + run.getTimePosted() +
					"\nTime Accepted = " + run.getTimeAccepted());
			System.out.println("-------------------------------");
		}
		...
		for(Run run : completed){
			System.out.println("Run Id = " + run.getRunId() + 
					...
					"\nTime Posted = " + run.getTimePosted() +
					"\nTime Accepted = " + run.getTimeAccepted() +
					"\nTime Completed = " + run.getTimeCompleted());
			System.out.println("-------------------------------");
		}
	}
